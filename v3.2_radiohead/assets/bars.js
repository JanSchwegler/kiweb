    ...
    barParent.forEach((e, i) => {
        if (barParent.length - 1 <= i) { // for last barParent
            for (let i = 0; i < Math.floor(height / 55); i++) {
                ...
                if (loadingAnimation) { // for resize, after initial load
...
function setAnimationForBars() {
    ...
    bars.forEach((e, i) => {
        ...
        let randomStart = Math.min((Math.random() * 41) + 60, 100);
        let randomEnd = Math.min((Math.random() * 41) + 60, 100);
        while (Math.abs(randomEnd - randomStart) < 15) {
            randomEnd = Math.min((Math.random() * 41) + 60, 100);
        }
